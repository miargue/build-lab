...
- **Page Sections:**
  ...
  4. **How It Works / Product Demo:** Problem: "A missed handover can delay real shipments." — Solution: "HandOff keeps the current state front and center." — outgoing lead updates the board throughout the shift — incoming lead reads the truth at a glance, no scrolling, no digging — Problem section includes an illustrative bar chart (minutes lost per task, placed on the left with the text on the right); Solution section includes an old-way vs. HandOff benefit table (illustrative times, clearly labelled as such)
...
| *2026-09-18* | *Problem / Solution visuals* | *Plain text vs. graph/table* | *Added an amber CSS bar chart to Problem (minutes lost per handover task) and a benefit table to Solution (old way vs. HandOff). Charts are CSS-only (no JS, within TECH.md). Same three tasks and illustrative numbers across both, and the note says "illustrative" so they are not presented as measured data. Reuse existing design tokens only. Founder then moved the Problem chart to the left of the text.* | *Done* |
| *2026-09-18* | *Scroll-reveal reliability* | *Leave as-is vs. more noticeable / cache-busting* | *User reported no visible reveal. Verified the code was being served correctly (a node stub proved the observer fires) — the causes were stale cached assets and/or OS `prefers-reduced-motion`. Fixes: cache-bust asset links (`?v=`), lower trigger threshold 0.15 -> 0.1, raise drift 12px -> 16px. Stays within the existing motion budget.* | *Done* |
...
- [x] **Feature: Landing Page + interim waitlist (Roadmap Stage 5) — COMPLETE (2026-09-18).** Synced spec in `SPECS/2026-09-17-base-shell/` (requirements.md, plan.md, validation.md). 5-Point Quality Audit: 0 Critical. 2026-09-18 follow-ups also done: scroll-reveal reliability fixes, 3 fake testimonial placeholders, Problem/Solution chart + table (chart moved to the left of the text per founder).
